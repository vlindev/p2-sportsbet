...
- **Bets entry/report UI optimization** 🔄 (S63–65) — Full UX/UI evaluation → 13 fixes → mockup v3 (S63) → v4 (S64) → v5 (S65). v5: typography applied (14px floor, 12px badges, 16px buttons), 分潤比例→選手佔成, per-player hero % for non-50/50, dropdown custom arrow, 編輯→pencil only. Full decision log: `memory/design-bets-entry-report-redesign.md`. **SIP Step 4 (mockup review) — run `uieval` next session, then SIP 5+6 if confident.**
