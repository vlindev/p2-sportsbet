...
- **Bets entry/report UI optimization** 🔄 (S63–64) — Full UX/UI evaluation → 13 fixes → mockup v3 (S63) → v4 (S64). v4 changes: 今日賽事 optgroup inside dropdown, 分潤比例 metric card with hero %, 封盤 iPhone toggle. Typography audit: 14px floor for readable text, 12px badges only, 16px action buttons — approved, NOT yet applied. Full decision log: `memory/design-bets-entry-report-redesign.md`. **SIP Step 4 (mockup review) — apply typo fixes, then continue review.**
